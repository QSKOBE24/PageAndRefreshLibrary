...
import com.guosen.library_pageandrefreshlistview.R;
...
public class PageAndRefreshListView extends RefreshListView implements
	...
	public PageAndRefreshListView(Context context, AttributeSet attrs,
		...
		mContext = context;
		TypedArray arr = getContext().obtainStyledAttributes(attrs, R.styleable.PageAndRefreshListView);
		mIsEnable = arr.getBoolean(R.styleable.PageAndRefreshListView_pageDemandingEnable, true);
		this.setRefreshable(arr.getBoolean(R.styleable.PageAndRefreshListView_refreshable, true));
		mNoDataGravity = arr.getInt(R.styleable.PageAndRefreshListView_noDataGravity, Gravity.CENTER);
		mNoLinkGravity = arr.getInt(R.styleable.PageAndRefreshListView_noLinkGravity, Gravity.CENTER);
		setNoDataImage(arr.getResourceId(R.styleable.PageAndRefreshListView_noDataImage, -1), mNoDataGravity);
		setNoLinkImage(arr.getResourceId(R.styleable.PageAndRefreshListView_noLinkImage, -1), mNoLinkGravity);
        arr.recycle();
	}
	...
	//在手指滑动过程，一直对于整个listview进行是否需要进行刷新和分页操作的监听
	@Override
	...
	public void setAdapter(ListAdapter adapter) {
		...
		if (adapter instanceof PageAndRefreshController) {
			...
			//设置好视图更新监听器，当在不同的操作下，对视图的更新进行监听
	 ...
	 * 设置好该ListView的 一些刷新过程中的头部视图和底部视图以及出错处理的视图
	 * @param context
	 */
