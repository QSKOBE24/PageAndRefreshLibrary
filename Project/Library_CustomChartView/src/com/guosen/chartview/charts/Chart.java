...
import com.guosen.chartview.animation.EasingFunction;
import com.guosen.chartview.component.Legend;
import com.guosen.chartview.component.MarkerView;
import com.guosen.chartview.data.ChartData;
...
import com.guosen.chartview.interfaces.ChartInterface;
import com.guosen.chartview.listener.ChartTouchListener;
import com.guosen.chartview.listener.OnChartGestureListener;
import com.guosen.chartview.listener.OnChartValueSelectedListener;
import com.guosen.chartview.render.DataRenderer;
import com.guosen.chartview.render.LegendRenderer;
...
public abstract class Chart<T extends ChartData<? extends DataSet<? extends Entry>>>
	 ...
	 */
	private float mDragDecelerationFrictionCoef = 0.95f;
	...
	/**
	 * = flag that indicates if the chart will show the default data 
	 */
	protected boolean showDefaultData = true;
	...
	/**
	 * listener that is called when the chart is selected
	 * */
	...
	private float mExtraTopOffset = 0.f, mExtraRightOffset = 0.f,
			mExtraBottomOffset = 0.f, mExtraLeftOffset = 0.f;
	/**
	 * default axis number count
	 * */
	protected int DefaultXCount = 7;
	...
	protected void init() {
		...
		mDefaultFormatter = new DefaultValueFormatter(3);
	...
	public void setData(T data) {
		if (data == null) {
			if (isShowDefaultData()) {
				ArrayList<String> xVals = new ArrayList<String>();
				int count = getDefaultXCount();
				for (int i = 0; i < count; i++) {
					xVals.add("" + i);
				}
				ArrayList<Entry> yVals = new ArrayList<Entry>();
				for (int i = 0; i < count; i++) {
					float val = (float) (Math.random() * 0.35) + 3.20f;
	...
	protected void drawDescription(Canvas c) {
		...
		if (!mDescription.equals("")) {
			...
			if (mDescriptionPosition == null) {
				...
				c.drawText(mDescription,
						getWidth() - mViewPortHandler.offsetRight() - 10,
						getHeight() - mViewPortHandler.offsetBottom() - 10,
						mDescPaint);
			} else {
				c.drawText(mDescription, mDescriptionPosition.x,
						mDescriptionPosition.y, mDescPaint);
			}
		}
	}
	 ...
	 * chart 图表上需要高亮显示的值对像
	 */
	public Highlight[] mIndicesToHightlight = new Highlight[0];
	...
	public boolean valuesToHighlight() {
		boolean flag = (mIndicesToHightlight == null
				|| mIndicesToHightlight.length <= 0
	...
	public void highlightValue(int xIndex, int dataSetIndex) {
		...
		if (xIndex < 0 || dataSetIndex < 0 || xIndex >= mData.getXValCount()
			...
			highlightValues(new Highlight[] { new Highlight(xIndex,
					dataSetIndex) });
		}
	}
	...
	/** if set to true, the marker view is drawn when a value is clicked */
	protected boolean mDrawDefaultMarkerViews = true;
	...
	protected void drawMarkers(Canvas canvas) {
		...
		for (int i = 0; i < mIndicesToHightlight.length; i++) {
			...
			if (xIndex <= mDeltaX && xIndex <= mDeltaX * mAnimator.getPhaseX()) {
				...
				if (e == null
						|| e.getXIndex() != mIndicesToHightlight[i].getXIndex())
					continue;
	 ...
	 * @return
	 */
	public abstract float[] getMarkerPosition(Entry e, int dataSetIndex);
	 ...
	 * @param durationMillisY
	 * @param easingX
	 *            a predefined easing option
	 * @param easingY
	 *            a predefined easing option
	 */
	public void animateXY(int durationMillisX, int durationMillisY,
			Easing.EasingOption easingX, Easing.EasingOption easingY) {
		mAnimator.animateXY(durationMillisX, durationMillisY, easingX, easingY);
	}
	...
	public float getAverage(String dataSetLabel) {
		...
		DataSet<? extends Entry> ds = mData.getDataSetByLabel(dataSetLabel,
				true);
	...
	public void setShowDefaultData(boolean showDefaultData) {
		this.showDefaultData = showDefaultData;
	}

	public int getDefaultXCount() {
		return DefaultXCount;
	}

	public void setDefaultXCount(int defaultXCount) {
		DefaultXCount = defaultXCount;
	}
	 ...
	 * @param enabled
	 */
	public void setDrawDefaultMarkerViews(boolean enabled) {
		mDrawDefaultMarkerViews = enabled;
	}
	...
	public boolean saveToGallery(String fileName, int quality) {
		...
		return getContext().getContentResolver().insert(
				Images.Media.EXTERNAL_CONTENT_URI, values) == null ? false
				: true;
	}
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		if (w > 0 && h > 0 && w < 10000 && h < 10000) {
			...
			if (mLogEnabled)
				Log.i(LOG_TAG, "Setting chart dimens, width: " + w
						+ ", height: " + h);
